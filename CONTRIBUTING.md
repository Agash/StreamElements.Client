# Contributing to StreamElements.Client
...
```bash
dotnet restore StreamElements.Client.slnx
dotnet build StreamElements.Client.slnx
dotnet test StreamElements.Client.slnx
```
...
```bash
dotnet test StreamElements.Client.slnx --filter "TestCategory!=Integration"
```
...
```
fix(realtime): reconnect after the server closes the socket
```
